 ...
 *
 * 距離寫「距查詢中心」,不寫「距比準地」也不寫「距區段中心」:metersToCenter 量到的是**查詢
 * 中心**,而那是哪一點要看呼叫端怎麼問 —— 帶了區段多邊形就是區段的幾何重心,沒帶才是比準地
 * 那一點(量到比準地的是另一個欄位 metersToPoint,本支不吃)。這份 facts 只拿到
 * byCategory/facilities、拿不到 area,無從分辨,所以用兩種情形都成立的說法。
 *
 * 這是進 prompt、再進交付文件的敘述,基準點寫死成其中一種,在另一種情形下就是一句不實的佐證。
 */
function formatFactsForPrompt(facts: CategoryFacts): string {
  ...
  return facts.facilities
    .map((f) => {
      ...
      const distPart =
        f.metersToCenter !== null ? `距查詢中心約 ${Math.round(f.metersToCenter)} M` : "距離未知";
