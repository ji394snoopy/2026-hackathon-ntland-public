...
import { normalizeRegionalAnalysisContent } from "./normalizeContent.js";
...
// input/regional-anlysis-<purpose>.pdf / coordinates-<purpose>.json off disk. All 5
// purposes are covered — the ASSETS[purpose] guard in the handler below is only a runtime
// safety net in case a template/coordinates inline ever comes back empty.
const ASSETS: Record<Purpose, { template: Uint8Array; coordinates: unknown }> = {
  agricultural: {
    template: templateAgricultural as Uint8Array,
    coordinates: coordinatesAgricultural,
  },
  commercial: {
    template: templateCommercial as Uint8Array,
    coordinates: coordinatesCommercial,
  },
  industrial: {
    template: templateIndustrial as Uint8Array,
    coordinates: coordinatesIndustrial,
  },
  other: {
    template: templateOther as Uint8Array,
    coordinates: coordinatesOther,
  },
  residential: {
    template: templateResidential as Uint8Array,
    coordinates: coordinatesResidential,
  },
};
...
// tree, accepted as the bare tree, { content }, or { content, purpose }.
// normalizeRegionalAnalysisContent additionally reshapes produce-regional-factors's own
// output shape into the content tree, for callers that post that directly instead of
// going through export-report first. A Function URL POST body may be base64-encoded
// (isBase64Encoded).
function parseRequest(event: FunctionUrlEvent): ParsedRequest {
  ...
  const purposeFromBody =
    parsed && typeof parsed === "object" ? parsed.purpose : undefined;
  ...
  return { purpose, content: normalizeRegionalAnalysisContent(content) ?? {} };
}
...
export async function handler(
  ...
  try {
    const assets = ASSETS[request.purpose];
    if (!assets) {
      return errorResponse(
        400,
        `regional-analysis template for purpose "${request.purpose}" is unavailable (bundled: ${Object.keys(ASSETS).join(", ")}).`,
      );
    }
    const { template, coordinates } = assets;
    ...
    const font = await pdfDoc.embedFont(fontBytes as Uint8Array, {
      subset: true,
    });
