...
import type { GradedResult, ResolvedGrade } from "../regionalFactorGrading/resolveGrades.js";
...
interface ItemRow {
  ...
  itemKey: string;
  itemRaw: string;
  base: ResolvedGrade | null;
  comparables: ComparableCell[];
}
...
interface FillReport {
  ...
  comparableSectionIds: ComparableSectionId[];
...
type GradedResultWithMeta = GradedResult & { meta: GradedResultMeta };
...
function flattenGradedResult(graded: GradedResult): Map<string, FlatItem> {
  ...
  for (const category of graded.regionalFactors.categories) {
...
function buildItemRow(
  ...
  const base = flatBase.get(entry.key)?.grade ?? null;
  const comparables: ComparableCell[] = flatComparables.map(({ label, flat }) => {
    const comparable = flat.get(entry.key) ?? null;
    return {
      label,
      grade: comparable?.grade ?? null,
      delta: base && comparable ? base.value - comparable.grade.value : null,
    };
  });
...
function buildCategoryTotals(
  ...
  const totalBase = items.reduce((sum, item) => sum + (item.base?.value ?? 0), 0);
  const comparableTotals = labels.map((label, index) => {
    const total = items.reduce((sum, item) => sum + (item.comparables[index]!.grade?.value ?? 0), 0);
...
function buildFillReport(a: GradedResultWithMeta, bEntries: LabeledGradedResult[]): FillReport {
  ...
  return {
    ...
    comparableSectionIds: bEntries.map(({ label, graded }) => ({
      label,
