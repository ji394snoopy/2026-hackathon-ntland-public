...
// 區域因素評的是整個地價區段的條件,基準點是「區段中心」,不是某一筆宗地。所以 survey 的
// items 同時帶兩個距離時,這裡一律只採 metersToCenter —— 讓模型自己在兩個數字間挑,它會挑
// 到 metersToPoint(表4 個別因素的基準),整張表五就以錯的基準點評級,而且看不出來。
const DISTANCE_BASIS_INSTRUCTION =
  "Distances: a survey field's `items[]` entries may carry TWO distances — `metersToCenter` " +
  "(to the 地價區段's centre) and `metersToPoint` (to this one parcel). 區域因素 grade the " +
  "SECTION, so ALWAYS use `metersToCenter` and ignore `metersToPoint` entirely. The same " +
  'applies to the `value` text: "金山國小，距150M（距比準地210M）" means 150m to the section ' +
  "centre — report 150, never the 比準地 number in parentheses. When only one distance is " +
  "present it is already the 區段中心 one.";
