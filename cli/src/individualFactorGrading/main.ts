...
const OUTPUT_DIR = "./src/individualFactorGrading/output";
const DEFAULT_SAMPLE_DATA_PATH =
  "./src/individualFactorGrading/input/sample-data.json";

async function main() {
  const [, , sampleDataPath = DEFAULT_SAMPLE_DATA_PATH] = process.argv;

  const { tool, promptText, individualFactors, meta, benchmark } =
    buildPipeline(sampleDataPath);

  console.log(`Invoking ${CLAUDE_MODEL_ID} (individualFactorGrading)...`);
  ...
  const graded = resolveGrades(individualFactors, toolInput.extractions, benchmark);
  writeFileSync(
