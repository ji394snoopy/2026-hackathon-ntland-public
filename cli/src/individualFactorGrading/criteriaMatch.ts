...
function matchBooleanOrAbsence(
  ...
  // named sub-fact exists on an otherwise range-typed (proximity) item. Absence of it
  // contributes nothing toward the worse end of that item's scale, so it's treated as
  // infinitely far away and resolved through the same range buckets as any other
  // distance.
...
function effectiveDistance(evidence: Evidence): number {
  switch (evidence.type) {
    ...
    case "enum":
      ...
      // range fact it always represents the item's nearest/best-defined state — 0 is
