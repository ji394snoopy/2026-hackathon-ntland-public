...
const OUTPUT_DIR = "./src/districtSurvey/specialFacilities/output";
...
// Every row in this category packs 名稱 and 本區段內/本區段外(距 M) onto a single line
// (nameY === distanceY for all 9 items, confirmed against coordinatePrompt.ts's
// REFERENCE_COORDINATES), so a filled-in facility name can overlap the printed 本區段內/外
// labels exactly like publicInfrastructure's wastewaterTreatmentFacility row did. When 名稱 is
// set we whiteout that whole line and redraw name + labels + circles + distance ourselves,
// walking an x-cursor by measured glyph width so nothing overlaps regardless of name length.
...
// calls, same split trafficAndTransport/publicInfrastructure use. This file just zips the two
// together at draw time.

const FACILITY_GROUP_KEYS = ["utilityGasFacility"] as const;
const CHECKED_FACILITY_GROUP_KEYS = ["funeralFacility", "wasteFacility"] as const;
...
function fillFacilityRow(page: PDFPage, item: any, row: any, font: PDFFont) {
  fillPackedNameRow(page, item.value, row, font);
}
...
function fillSpecialFacilitiesContent(
  page: PDFPage,
  font: PDFFont,
  ...
  for (const key of FACILITY_GROUP_KEYS) {
    const items = input[key].items as any[];
    const rows = coordinates[key] as any[];
    items.forEach((item, i) => {
      if (!rows[i]) {
        console.warn(`specialFacilities: missing coordinates for ${key} item ${i}, skipping`);
        return;
      }
      fillFacilityRow(page, item, rows[i], font);
    });
  }
...
export { fillSpecialFacilitiesContent };

if (process.argv[1] === fileURLToPath(import.meta.url)) {
  runStandaloneFill({
    outputDir: OUTPUT_DIR,
    fillContent: fillSpecialFacilitiesContent,
  }).catch((err) => {
    console.error(err);
    process.exit(1);
  });
